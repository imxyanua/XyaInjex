...
- **`--write-corpus`** (with `--evolve`, dev-only): append discoveries to
  `corpus/{lang}.py`, reload the corpus module, run benchmark, and revert the
  file if regression fails.
...
- **Evolve web panel**: run parser-divergence evolution from the UI with configurable
  rounds, per-discovery differential drill-down, and MD/JSON export.
- **`--emit-corpus`** on CLI and **`emit_corpus`** on `POST /evolve`: emit review-ready
  `CorpusCase(...)` Python snippets for novel discoveries.
- **Encode variants in evolve loop**: each round also tests encoded payload variants
  (`encode:<strategy>`) alongside fuzz paths.
...
- POSIX shell analysis adjusts **risk** when tree-sitter bash disagrees with the
  ...
  false positives (e.g. leading `||` or `|`) are downgraded one step.
...
[Unreleased]: https://github.com/imxyanua/XyaInjex/compare/v0.18.1...HEAD
[0.18.1]: https://github.com/imxyanua/XyaInjex/compare/v0.18.0...v0.18.1
